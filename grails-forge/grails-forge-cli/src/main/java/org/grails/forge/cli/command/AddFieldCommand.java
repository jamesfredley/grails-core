...
/**
 * CLI command to add a field to an existing domain class.
 *
 * @since 7.0
 */
@Command(name = AddFieldCommand.NAME, description = "Adds a field to an existing domain class")
public class AddFieldCommand extends CodeGenCommand {
    ...
    public Integer call() throws Exception {
        ...
        if (constraintStyle != null) {
            try {
                field.setConstraintStyle(FieldDefinition.ConstraintStyle.valueOf(constraintStyle.toUpperCase()));
            } catch (IllegalArgumentException e) {
        ...
        if (domainFieldModifier.fieldExists(domainFile, field.getName())) {
        ...
        try {
            ...
            out("@|blue ||@ Added field '" + field.getName() + "' to " + getRelativePath(projectDir, domainFile));
        } catch (Exception e) {
