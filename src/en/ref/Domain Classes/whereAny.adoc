...
The `where` method is a powerful new type-safe querying option introduced in Grails 2.0. For more information on using the `where` method see the dedicated section on https://gorm.grails.org/{gormVersion}/hibernate/manual/index.html#whereQueries[Where Queries] and https://gorm.grails.org/{gormVersion}/hibernate/manual/index.html#detachedCriteria[Detached Criteria] in the user guide.
