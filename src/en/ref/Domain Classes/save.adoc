...
* `flush` (optional) - When set to `true` flushes the persistence context, persisting the object immediately and updating the `version` column for https://gorm.grails.org/{gormVersion}/hibernate/manual/index.html#locking[optimistic locking]
