...
* `XML` - An instance of `XmlSlurper`'s https://docs.groovy-lang.org/latest/html/gapi/groovy/xml/slurpersupport/GPathResult.html[GPathResult] class that allows parsing of an incoming XML request (useful for REST).
* `JSON` - An instance of Grails' https://docs.grails.org/3.2.x/apiorg/codehaus/groovy/grails/web/json/JSONObject.html[JSONObject] class that allows parsing of an incoming JSON request (useful for JSON based REST).
