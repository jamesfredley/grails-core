...
description("Adds a field to an existing domain class") {
    usage "grails add-field [DOMAIN CLASS] [FIELD:TYPE]"
...
if (args.size() < 2) {
    error "Usage: grails add-field [DOMAIN CLASS] [FIELD:TYPE]"
    error "Example: grails add-field Book title:String --nullable"
    return false
}
...
if (constraintStyleFlag != null) {
    try {
        field.constraintStyle = FieldDefinition.ConstraintStyle.valueOf(constraintStyleFlag.toString().toUpperCase())
    } catch (IllegalArgumentException e) {
...
if (modifier.fieldExists(domainFile, field.name)) {
...
try {
    ...
    addStatus "Added field '${field.name}' to ${projectPath(sourceClass)}"
} catch (Exception e) {
