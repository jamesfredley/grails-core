...
TIP: If you are interested in more examples, an https://github.com/grails/grails-data-mapping/blob/{version}/grails-data-mongodb/gorm/src/main/groovy/org/grails/datastore/mapping/mongo/connections/MongoConnectionSources.groovy[implementation exists for MongoDB] that reads connection sources from a MongoDB collection. However, it doesn't implement support for provision MongoDB instances at runtime.
