...
Setting the `dbCreate` setting to "none" is recommended once your schema is relatively stable and definitely when your application and database are deployed in production. Database changes are then managed through proper migrations, either with SQL scripts or a migration tool like https://flywaydb.org/[Flyway] or http://www.liquibase.org/[Liquibase]. The https://gorm.grails.org/{gormVersion}/hibernate/manual/index.html#databaseMigration[Database Migration] plugin uses Liquibase.
