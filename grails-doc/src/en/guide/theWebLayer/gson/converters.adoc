...
It is possible to register custom converters to change how a given class is rendered with json views. To do so, create a class that implements link:{api}groovy.json.JsonGenerator.Converter.html[Converter]. Then you must register the class in `src/main/resources/META-INF/services/groovy.json.JsonGenerator$Converter`.
