...
Use the xref:../../ref/Tags%20-%20GSP/render.adoc[render] tag to render this template from one of the views in `grails-app/views/book`:
...
You can also render templates from controllers using the xref:../../../ref/Controllers/render.adoc[render] controller method. This is useful for JavaScript heavy applications where you generate small HTML or data responses to partially update the current page instead of performing new request:
...
The link:xref:../ref/Controllers/render.adoc[render] controller method writes directly to the response, which is the most common behaviour. To instead obtain the result of template as a String you can use the xref:../ref/Tags%20-%20GSP/render.adoc[render] tag:
...
Notice the usage of the `g` namespace which tells Grails we want to use the xref:tags/tagsAsMethodCalls.adoc[tag as method call] instead of the xref:../../../ref/Controllers/render.adoc[render] method.
