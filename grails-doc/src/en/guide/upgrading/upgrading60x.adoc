...
JSON rendering of `java.util.Calendar`, `java.time.Instant`, `java.time.LocalDateTime`, `java.time.OffsetDateTime`, and `java.time.ZonedDateTime` has been updated for consistency across both standard JSON converters (`render ... as JSON`) and JSON views (`.gson` files).
...
If your application or API consumers depend on the previous JSON format for `Calendar`, `Instant`, `LocalDateTime`, `OffsetDateTime`, or `ZonedDateTime` fields:
...
2. **Date Parsing**: The new format is a standard ISO-8601 string that can be parsed using `Instant.parse()`, `LocalDateTime.parse()`, `OffsetDateTime.parse()`, or `ZonedDateTime.parse()` with appropriate formatters.

3. **Consistency**:
   - Temporal types with timezone information (`Date`, `Calendar`, `Instant`) render with **Z suffix** (UTC)
   - `LocalDateTime` (which has no timezone) renders **without suffix**
