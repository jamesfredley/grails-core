...
7. Several static variables on `ClosureEventTriggeringInterceptor` are removed - use the variables on `AbstractPersistentEvent` instead.
8. `org.grails.plugins.databinding.DataBindingGrailsPlugin` was removed - use `DataBindingConfiguration` instead.
