...
`GrailsExceptionResolver` now appends a parenthesised clause of per-request context — the remote client address
and the current auditor — to its exception log headline:
...
need to tolerate the optional ` (ip: <value>, user: <value>)` suffix. Both pieces of context are enabled by
default. Opt out individually:
...
grails:
    exceptionresolver:
        ...
        logRemoteAddr: false    # drop ' ip: <value>'
----

When neither applies to a given request — no `AuditorAware` bean registered and the IP suffix disabled — the
